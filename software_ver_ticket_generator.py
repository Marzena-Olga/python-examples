...
# Example Platform Operations  platform_ops

def send_mail(to_email='contact5@example.com', subject='Monthly upgrade version ticket generator', message='', server='host11.example.com',
              from_email='contact2@example.com', passwd=''):
    msg = EmailMessage()
    msg['Subject'] = subject
    msg['From'] = from_email
...
def get_jira_data(key=''):  # function to test connection to jira
    ...
    proxy = {'http': '', 'https': ''}
    url = 'https://host12.example.com/rest/api/latest/search?jql=project={0}'.format(key)
    # url = 'https://host12.example.com/rest/api/latest/issue/OPS-100'
    url = 'https://host12.example.com/rest/api/latest/issue/OPS-100'
...
def create_jira_ticket(fields_json):  # function create jira ticket
    ...
    headers = {'Content-Type': 'application/json', 'Authorization': 'Bearer %s' % encoded_tok}
    proxy = {'http': '', 'https': ''}
    url = 'https://host12.example.com/rest/api/2/issue/'
    ...
    if response.status_code == 201:
        ticket = result['key']
        ticket_url = ('https://host12.example.com/browse/{0}').format(ticket)
        print(ticket_url)
        return ticket_url
    else:
        print("No ticket")
...
def prepare_jira_json(key, table_dict, project):  # function prepare jira json with summary, description and reporter
    ...
    assignee = 'EXAMPLEUSER002'
    a_name = 'platform_ops'
    # print(assignee)
    if project.find('CLIENT_A-') != -1:
        assignee = 'EXAMPLEUSER004'  # team_a_ops
        a_name = 'team_a_ops'
    if project.find('CLIENT_B-') != -1:
        assignee = 'EXAMPLEUSER001'  # team_b_ops
        a_name = 'team_b_ops'
    if project.find('CLIENT_C-') != -1:
        assignee = 'EXAMPLEUSER005'  # team_c_ops
        a_name = 'team_c_ops'
    if project.find('CLIENT_D-') != -1:
        assignee = 'EXAMPLEUSER006'  # team_d_ops
        a_name = 'team_d_ops'
                ...
                "assignee":
                    {
                        ...
                        "emailAddress": "contact5@example.com"
                    },
                "priority": {"id": "10000"},
        ...
        "fields":
            {
                "parent": {
                    "id": "10001",
                    "key": "OPS-100",
                    "self": "https://host12.example.com/rest/api/2/issue/10001",
                    "fields": {
                        "summary": "CLIENT_A/CLIENT_C/CLIENT_D/CLIENT_B  monthly application version check",
                        "priority": {"id": "10"},
                        "issuetype": {"name": "Task"}
                    },
                },
                ...
                "assignee":
                    {
                        "key": assignee,  # "EXAMPLEUSER002",
                        "name": a_name  # "platform_ops",
                    },
                "priority": {"id": "10"},
                "summary": ('{0} Application version check').format(project),
                "description": final_description,
...
def get_prom_data2(metric, prometheus_url):  # function get metrich from Prometheus (as json) by REST-API
    ...
    z = json.loads(z)
    # url = 'https://host09.example.com:8443/api/v1/query?query=node_app_available_version_info'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Script required path eq:  --token=jira_token --url=https://host09.example.com:8443/ --login=admin --project=(CLIENT_A-TEST) --password=EXAMPLE_PASSWORD --smtp_pass=(SMTP-PASS)"
    )
    ...
    if (m1 == 0) or (m2 == 0):
        ...
        print(message,' ',email )
        #send_mail(to_email='contact5@example.com', subject='Monthly upgrade version ticket generator', message=message, server='host11.example.com', from_email=email, passwd=smtp_pass )
        r = -1
    else:
